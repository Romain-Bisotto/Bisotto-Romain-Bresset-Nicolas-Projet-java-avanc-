...
import main.java.fr.dauphine.JavaAvance.Solve.Checker;
import main.java.fr.dauphine.JavaAvance.Solve.Solver;
...
public class Grid {
	...
	public ArrayList<Piece> listOfFixedNeighbours(Piece p){
		ArrayList<Piece> lp = new ArrayList<>();
		for (Piece piece: listOfNeighbours(p)){
			if (piece.isFixed()){
				lp.add(piece);
			}
		}
	...
	public Piece getNextPiece(Piece p) {
		...
		if (j < this.getWidth() - 1) {
			nextPiece = this.getPiece(i, j + 1);
		}
		else {
			if (i < this.getHeight() - 1) {
				nextPiece = this.getPiece(i + 1, 0);
			} else {
				return null;
			}
	...
	public int[] getNextCoordonne(int i, int j){
	...
	public boolean isConnected(Piece p, Orientation ori) {
		...
		if(ori == Orientation.NORTH){
			oppPieceX = p.getPosX();
			oppPieceY = p.getPosY() - 1;
		}
		if (ori == Orientation.EAST){
			oppPieceX = p.getPosX() + 1;
			oppPieceY = p.getPosY();
		}
		...
		if (p.getType() == PieceType.VOID)
			return true;
		if(oppPieceX < 0 || oppPieceY < 0 || oppPieceX >= width || oppPieceY >= height){
			return false;
		}
		try {
	...
	public boolean isConnected(Piece p, Orientation ori, boolean reverse) {
		int oppPieceY = 0;
		int oppPieceX = 0;
		if (reverse){
			if(ori == Orientation.NORTH){
			...
			oppPieceX = ori.getOpposedPieceCoordinates(p)[0];
			oppPieceY = ori.getOpposedPieceCoordinates(p)[1];
		}
		if (p.getType() == PieceType.VOID)
			return true;
		if(oppPieceX < 0 || oppPieceY < 0 || oppPieceX >= width || oppPieceY >= height){
	...
	public boolean isTotallyConnected(Piece p, boolean reverse) {
		if (p.getType() != PieceType.VOID) {
			for (Orientation connector : p.getConnectors()) {
				if (!this.isConnected(p, connector, reverse)) {
					return false;
				}
			}
		}
	...
	public void turnPiece(int i, int j, Grid grid, GameControler gameControler){
		pieces[i][j].turn();
		if(Checker.isSolve(grid, gameControler)){
			gameControler.victoire();
			return;
		}
		return;
	}

	public static Grid solveGraphic(GameControler gameControler, Grid grid){
		Grid gridSolv = Solver.solverGraphic(grid, gameControler);
		if (gridSolv != null){
			return grid;
		}
		else {
			gameControler.impossible();
			return null;
		}
